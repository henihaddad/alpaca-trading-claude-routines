...
=== STEP 2: SIGNALS ===
`cd repo && python3 -m pipeline.collect --hours 96 --out data/live && python3 -m pipeline.signals --raw data/live --out data/live/hourly.json && python3 -m pipeline.brief --features data/live/hourly.json --raw data/live --held <comma-separated symbols you hold> --out data/live/brief.md` (stdlib only, ~3-5 min; STOCKTWITS_MAX_PAGES=40 env var keeps it fast). The brief ranks the whole 151-symbol universe in pipeline/universe.py and shows the top 20 by score (24h growth x log mentions) plus your held positions. If it fails, use the committed repo/data/live/brief.md if present and say so; with no brief at all, treat every symbol as failing entry condition (a).
Read the brief. Then for every held symbol and every symbol in the brief's top 20 that passes entry condition (a) below, get 30 daily bars in one call: GET https://data.alpaca.markets/v2/stocks/bars?symbols=AAPL,MSFT,...&timeframe=1Day&limit=30&feed=iex (response is {"bars": {"AAPL": [...], ...}}); crypto GET https://data.alpaca.markets/v1beta3/crypto/us/bars?symbols=BTC%2FUSD&timeframe=1Day&limit=30. Compute the 20-day high and low, distance from the 20-day high, 5-day return. WebSearch only to verify a specific catalyst the brief surfaced.
...
INSTRUMENTS: any symbol in pipeline/universe.py (S&P 100 + liquid retail favourites, SPY/QQQ/VOO/IWM/DIA, and the Alpaca-listed cryptos). When more candidates pass than free slots, take the highest brief score first. No options, penny stocks, leveraged or inverse ETFs.
...
Legacy positions without a live stop: create one now at the stop in state/journal.md (BTC/USD 78000 stop_limit; QQQ 715; SPY 757); for crypto other than BTC/ETH use the same stop_limit mechanics, with client_order_id "v2stop_<SYM>_<YYYYMMDD>". Cancel stale open orders that no longer make sense. Read back /v2/orders to confirm everything.
